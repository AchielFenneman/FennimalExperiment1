...
#### `morph_task`
Speeded 2AFC identity DV on an **extra-wide indoor polaroid** (photo-room stage: Home overlay, keyboard only, twin time bars, hidden scoring). One shared photo well holds a smaller **prime** (back-left, head + hat) and a larger **jumble** (front-right, two heads). Both start with the prime hat already showing (head `[?]`). Jumble starts under a light-gray `[?]`. Caption stays `????`. No resolve / no identity feedback. Always true-grayscale.

**Trial flow**
1. Polaroid: prime hat visible, head `[?]` (white drop-shadow during the name quiz); jumble under a light-gray `[?]`. Prime is drawn on top.
2. Radial **name quiz** around the prime (white drop-shadow chips; **F** / **J** move, **Space** confirm). Wrong → hide ring, shake, retry (unpaid). Correct → head `[?]` snaps off.
3. Pause `primeRevealHoldMs` (1000 ms), then the jumble `[?]` fades out over `jumbleFadeMs` (1400 ms) with the jumble drawn on top. Mix is `mix`% target / (100−`mix`)% distractor. Score clock + hat 2AFC arm after the fade.
4. **F / J** keycaps show the two jumble parents' hats (prime hat excluded). Polaroid **flies** to the chosen side; scene fades. No morph-to-truth.

**Practice** (unless `skip_practice`): two unpaid shape trials on the same two-spot polaroid. Left: a circle (no hat); the `[?]` lifts before the name quiz. Name quiz options Circle / Diamond / Star (correct = Circle). Right: an 80/20 square–triangle mix (`practiceMix`). Identity keys show the two shapes. The first practice trial has coaching bubbles (F/J name quiz, mix, time bars). The first paid trial has a short “same two steps…” bubble, then an F/J hat-choice bubble.
...
| `trial_speed` | Identity window ms after jumble fade (default 5000). Late answers still required; remaining points hit 0. |
| `skip_practice` | If `true`, omit the two shape-practice trials. Default / unset / `false` keeps them. The first paid trial still gets a short hat-key layout bubble. |
| `day_title` / `day_body` | Optional Day N overrides. |
| `skip_instructions` | Skip Day N page. |
| `partner_behavior` | Use `"absent"`. |

**Rendering:** both parents are rasterized, then **size-normalized from the opaque silhouette** (equal printed max-side) and rotated/translated from the eyes so mix `.50` is symmetric (no z-order). Face-only similarity is not used — that was scaling small-faced heads (e.g. stocking) up to match large-faced ones. `crossfade` is a premultiplied RGB lerp on those aligned rasters. `mesh` resamples homologous rays after alignment (contour + inner ring), warps both to `dest(m)`, lerps, and seals remaining cracks (never additive `lighter`). `silhouette` lerps a lightly blurred signed-distance field, anti-aliases with `clamp(0.8·sdf+0.5,0,1)`, and replaces low-alpha lerp fringes with mean opaque gray. No noise, blur film, or overlay SVG stacking.

**Order:** two unpaid shape-practice trials (unless `skip_practice`), then the assigned morph’s mix × pair × target list shuffled within. F/J hat sides persist on `morph_button_sides`. Name-chip order persists on `morph_prime_button_order`. Trial ids: `{morph}_p{prime}_{fenA}-{fenB}_t{target}_m{mix}` (e.g. `crossfade_pA_C-D_tC_m50`). `fenA`/`fenB` are the pair labels; `target` is the majority parent and `distractor` is the other. Both pair members are generated as targets.
...
#### `morph_task_two_stage_development` (archived)
Snapshot of the single-polaroid developing-photo morph (composite prime in the same well → timed prime→jumble morph with noise → fenA/fenB morph that resolves over `trial_speed`). Kept loadable via `MorphTaskTwoStageDevelopmentController` (`4_MorphTaskTwoStageDevelopment.js`) / `GenParam.MorphTaskTwoStageDevelopment` for replay; **not used by live structures**. Prefer `morph_task`.
