...
class BasicElementsModule {
    ...
    async perform_success_celebration(TargetBoxElement) {
        // 1. Calculate the distance to move next to the target object
        let boxCenter = getSVGInternalCenter(TargetBoxElement);
        let fenCenter = getSVGInternalCenter(this.Fennimal);

        // Move to about 150px to the side so they don't overlap awkwardly
        let offset = fenCenter.x < boxCenter.x ? -150 : 150;
        let targetX = boxCenter.x + offset;
        ...
        // FIX: Directly use the native gaze controller!
            ...
            particleInterval = setInterval(() => {
                for(let p = 0; p < 2; p++) {
                    ...
                    new SmallFeedbackSymbol(
                        ...
                        symbol,
                        1000,
                        (targetX + randomOffsetX), (fenCenter.y - 150 + randomOffsetY),
                        (targetX + randomOffsetX), (fenCenter.y - 320 + randomOffsetY)
                    );
                }
            }, 120);
        }

        // 4. Do happy jumps around the box
        await this.Fennimal_jump(100);
        await this.Fennimal_move_relative(600, 0, 500);
        await this.Fennimal_jump(150);
        await this.Fennimal_jump(150);
        await this.Fennimal_move_relative(-600, 0, 500);
        await this.Fennimal_jump(100);


        // Shut off the fountain
        if (particleInterval) clearInterval(particleInterval);

        // FIX: Cleanly release the gaze using your native function
...
class FlySwatTrialController {
    ...
    async finish_trial() {
        // FIX: Removed the old, redundant jumping sequence!
    ...
    async start_sequence() {
        ...
        let numFlies = this.partner.is_present ? 2 : 3; //10 & 7
