class InstructionsController {
    ...
    addClosingButtonToParent(position, addKeyboardShortcut, optionalAdditionalFunction, optionalDelayTime) {
        switch (position) {
            case "top-right":
                ...
                break;
            case "top-left":
                this.closingButton = create_SVG_buttonElement(2 * this.boundarySize, 3 * this.boundarySize, 75, 75, "X", 70);
                break;
            case "bottom-center":
    ...
    startFennimalAttributeSortingTask(phaseNum, taskData, attributesArr, maxEarnableStars) {
        ...
        let rewardInstruction = maxEarnableStars > 0 ? "<br><br><br><br><br>Please pay close attention while answering the questions. You will start the day with " + maxEarnableStars + " bonus stars - but you will lose one star for each mistake you make!" : "";

        let instructionText = taskInstruction + rewardInstruction;
        this.textElemMainInstructions = create_SVG_text_in_foreign_element(instructionText, 0.1 * GenParam.SVG_width, 0.15 * GenParam.SVG_height, 0.8 * GenParam.SVG_width, 0.6 * GenParam.SVG_height, "instruction_element_text");
        this.textElemMainInstructions.classList.add("instruction_element_nonbackground");
        this.textElemMainInstructions.getElementsByClassName("instruction_element_text")[0].style.fontSize = "40px";
        this.currentInstructionsSVG.appendChild(this.textElemMainInstructions);
    ...
    showCompletionCodeScreen() {
        ...
        let text = "Do NOT close or refresh this window before submitting your code to Prolific. <br>" +
            " Your completion code is: <tspan style='user-select:all'><b> " + this.expPaymentData.completion_code + " </b></tspan>. <br>" +
            "<br> " +
...
class PaymentCard {
    constructor(parent, dayData) {
        ...
        let starsHaveBeenEarned = dayData.stars_earned > 0;
        let isSummaryCard = dayData.day_type === "summary";
        ...
        setTimeout(() => {
            let amountText = create_SVG_text_elem(0.5 * paymentCardWidth, 0.8 * paymentCardHeight, "×" + dayData.stars_earned, "instruction_element_text");
        ...
        setTimeout(() => {
            let totalText = create_SVG_text_elem(0.4 * paymentCardWidth, 0.95 * paymentCardHeight, "(out of " + dayData.maximum_possible_stars + ")", "instruction_element_text");
