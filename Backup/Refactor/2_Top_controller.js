    ...
    getPaymentData() {
        ...
        this.experimentData.paymentData = {
            phases: [...this.paymentInfo, {
                ...
                maximumPossibleStars: this.stimuli.get_maximum_number_of_bonus_stars()
            }],
...
class TrialGenerator {
    ...
    generateTrialsForPhase(phaseData) {
        ...
        // Distribute interaction types pseudo-randomly to avoid back-to-back same-Fennimal encounters
        const ordering = pseudo_randomize_order_of_ids_no_back_to_back(
            get_all_values_in_array_of_objects("id", baseFennimalSet),
            interactionTypesArr.length
        );
        ...
        if (phaseData.type === "hint_and_search") {
            ...
            trialSet = set_property_to_all_elem_in_arr("hint_type", hintTypeArr[0], trialSet);
        }

        // Merge Orthogonal Tasks
        if (phaseData.included_orthogonal_tasks) {
            let orthogonalTrials = this.getOrthogonalTaskTrials(phaseData);
            let protectedCount = phaseData.orthogonal_tasks_possible_after_trial || 0;
            trialSet = this.mergeTrialSets(trialSet, orthogonalTrials, protectedCount);
        }
    ...
    mergeTrialSets(baseArray, insertArray, protectedCount) {
